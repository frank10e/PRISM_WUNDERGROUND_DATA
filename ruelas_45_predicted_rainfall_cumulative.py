...
file_path = r"C:\Users\User\OneDrive\桌面\Wunderground Data\ruelas_45_predicted_rainfall.csv"
data_long = pd.read_csv(file_path)

# Convert 'Date' to datetime
data_long['Date'] = pd.to_datetime(data_long['Date'], format='%Y-%m-%d')
data_long['Predicted_Rainfall'] = np.where(data_long['ppt_prism'] == 0, 0, data_long['Predicted_Rainfall'])
...
for station in data_filtered['Name'].unique():
    ...
    for end_date in end_dates:
        cumulative_precip = station_data[(station_data['Date'] >= march_start) & 
                                         (station_data['Date'] < end_date)]['Predicted_Rainfall'].sum()
...
output_path = r"C:\Users\User\OneDrive\桌面\Wunderground Data\ruelas_45_predicted_rainfall_cumulative.xlsx"
